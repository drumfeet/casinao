...
Handlers.add('deal', Handlers.utils.hasMatchingTag('Action', 'Deal'), function(msg)
    ...
    local success, err = pcall(function()
        ...
        printData("playerHand", playerHand)
        printData("dealerHand", dealerHand)
        ...
        printData("playerValue", playerValue)
        printData("dealerUpCardValue", dealerUpCardValue)

        -- Check if the dealer's up card is an Ace (insurance option)
        local insuranceOption = false
        if dealerHand[1].rank == 'A' then
            insuranceOption = true
        end
        printData("insuranceOption", insuranceOption)

        -- Check for player's natural Blackjack
        local playerHasBlackjack = (playerValue == 21 and #playerHand == 2)
        printData("playerHasBlackjack", playerHasBlackjack)

        -- If the player has a natural Blackjack, check if it's a push or win
        if playerHasBlackjack then
            -- Check if dealer also has Blackjack (not yet revealed, but stored for next action)
            local dealerHasBlackjack = false -- This will be calculated when the dealer reveals their second card

            -- End the game if a push or win
            -- The dealer's second card will need to be revealed after the player completes their turn
            -- so we can determine if it's a push or the player wins outright.
        end

        local gameState = {
            PlayerHand = playerHand,
            DealerHand = dealerHand, -- Only one card shown for the dealer
        ...
        -- Send a message back to the user containing the game state
        ao.send({
            Target = msg.From,
